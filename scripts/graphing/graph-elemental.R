...
elemental_line <- function(
  ...
  if(!smoothed){
    g <- g + geom_line(aes_string(group="subject_id"), size=line_size, color=scales::alpha(color_name,line_alpha), na.rm=T)
  } else{
    g <- g + geom_smooth(aes_string(group="subject_id"),size=line_size,  method="lm",color=scales::alpha(color_name,line_alpha), na.rm=T, se=F )
